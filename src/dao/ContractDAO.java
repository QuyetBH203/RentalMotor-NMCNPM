...
public class ContractDAO extends DAO {
    ...
    public boolean addContract(Contract b) {
        ...
        String addRentalDetail
                = "INSERT INTO tblrentaldetail(price, startDate, endDate, tblMotorId, tblContractId,desDamages,numberOfDamages)"
                + "VALUES(?,?,?,?,?,?,?)";
...
//                    pa.setInt(1, 1);
                    pa.setDouble(1, x.getMotor().getPrice());
                    pa.setString(2, formatter.format(x.getStartDate()));
                    pa.setString(3, formatter.format(x.getEndDate()));
                    pa.setInt(4, x.getMotor().getId());
                    pa.setInt(5, b.getId());
                    pa.setString(6, x.getDesDamage());
                    pa.setInt(7, 1);
                    pa.executeUpdate();
                    //get id of the new inserted booking
